...
use pyo3::{
    FromPyObject, IntoPyObject, Py, PyAny, PyErr, PyResult, Python,
...
impl EngineAPI2 {
    ...
    pub fn create_window(&self, options: PyValue) -> Result<String, PyErr> {
        let mut guarded_engine = self.engine.lock().unwrap();
        let options = options.parse_to::<CreateWindowParams>()?;
        match options {
    ...
    pub fn close_window(&self, window_id: String) -> Result<(), PyErr> {
        ...
        if let Some(popup_layout) = guarded_engine.popups.as_mut()
            && popup_layout.layout.find_child(window_id.clone()).is_some()
        {
            if let Some(new_layout) = popup_layout.layout.clone().remove_window(&window_id) {
                popup_layout.layout = new_layout;
            }

            guarded_engine.popups = None;
        }

        let has_window = guarded_engine
            .layout
            .as_mut()
            .map_or(false, |l| l.find_child(window_id.clone()).is_none());

        if has_window && let Some(old_layout) = &mut std::mem::take(&mut guarded_engine.layout) {
